// Vendored from https://github.com/RossWilliams/ts-case-convert/blob/bfe6cb8151dc461f10a3f13bfa195af8e9e464c1/src/caseConvert.ts
// with methods added to convert to dash case (toDash, objectToDash, ObjectToDash).

// Added methods to convert to dash case.
...
function convertObject<
  ...
  TResult extends ObjectToCamel<TInput> | ObjectToSnake<TInput> | ObjectToPascal<TInput> | ObjectToDash<TInput>,
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    out[keyConverter(k)] = Array.isArray(v)
      ? (v.map(<ArrayItem extends object>(item: ArrayItem) =>
          typeof item === 'object' && !(item instanceof Uint8Array) && !(item instanceof Date)
            ? convertObject<
                ArrayItem,
...
export function toSnake<T extends string>(term: T): ToSnake<T> {
  ...
  while ((/([a-z])([0-9])/.exec(result)?.length || 0) > 2 && circuitBreaker < 10) {
    result = result.replace(
      /([a-z])([0-9])/,
      (_all, $1: string, $2: string) => `${$1.toLowerCase()}_${$2.toLowerCase()}`,
    );
...
export function toDash<T extends string>(term: T): ToDash<T> {
  let result: string = term;
  let circuitBreaker = 0;
  ...
  while ((/([a-z])([0-9])/.exec(result)?.length || 0) > 2 && circuitBreaker < 10) {
    result = result.replace(
      /([a-z])([0-9])/,
      (_all, $1: string, $2: string) => `${$1.toLowerCase()}-${$2.toLowerCase()}`,
    );
  ...
  return result.toLowerCase() as ToDash<T>;
}

export function objectToDash<T extends object>(obj: T): ObjectToDash<T> {
  return convertObject(obj, toDash);
}
...
export function toPascal<T extends string>(term: T): ToPascal<T> {
  return toCamel(term).replace(/^([a-z])/, m => (m[0] || '').toUpperCase()) as ToPascal<T>;
}
...
export type ObjectToCamel<T extends object | undefined | null> = T extends undefined
  ? undefined
  : T extends null
    ? null
...
export type ObjectToPascal<T extends object | undefined | null> = T extends undefined
  ? undefined
  : T extends null
    ? null
    : T extends Array<infer ArrayType>
      ...
      : T extends Uint8Array
        ...
        : T extends Date
          ? Date
          : {
              [K in keyof T as ToPascal<K>]: T[K] extends Array<infer ArrayType> | undefined | null
                ? ArrayType extends object
                  ? Array<ObjectToPascal<ArrayType>>
                  : Array<ArrayType>
                : T[K] extends object | undefined | null
...
export type ToDash<S extends string | number | symbol> = S extends string
  ? S extends `${infer Head}${CapitalChars}${infer Tail}` // string has a capital char somewhere
    ? Head extends '' // there is a capital char in the first position
      ? Tail extends ''
        ? Lowercase<S> /*  'A' */
        : S extends `${infer Caps}${Tail}` // tail exists, has capital characters
          ? Caps extends CapitalChars
            ? Tail extends CapitalLetters
              ...
              : Tail extends `${CapitalLetters}${string}`
                ? `${ToDash<Caps>}-${ToDash<Tail>}` /* first tail char is upper? 'ABcd' */
                : `${ToDash<Caps>}${ToDash<Tail>}` /* 'AbCD','AbcD',  */ /* TODO: if tail is only numbers, append without underscore */
            : never /* never reached, used for inference of caps */
          : never
      : Tail extends '' /* 'aB' 'abCD' 'ABCD' 'AB' */
        ? S extends `${Head}${infer Caps}`
            ...
            ? Head extends Lowercase<Head> /* 'abcD' */
              ? Caps extends Numbers
                ? // Head exists and is lowercase, tail does not, Caps is a number, we may be in a sub-select
                  ...
                  : // head does not end in number, safe to split. 'abc2' -> 'abc_2'
                    `${ToDash<Head>}-${Caps}`
                : `${ToDash<Head>}-${ToDash<Caps>}` /* 'abcD' 'abc25' */
              : never /* stop union type forming */
            : never
          : never /* never reached, used for inference of caps */
        : S extends `${Head}${infer Caps}${Tail}` /* 'abCd' 'ABCD' 'AbCd' 'ABcD' */
          ? Caps extends CapitalChars
            ? Head extends Lowercase<Head> /* is 'abCd' 'abCD' ? */
              ? Tail extends CapitalLetters /* is 'abCD' where Caps = 'C' */
                ? `${ToDash<Head>}_${ToDash<Caps>}-${Lowercase<Tail>}` /* aBCD Tail = 'D', Head = 'aB' */
                : Tail extends `${CapitalLetters}${string}` /* is 'aBCd' where Caps = 'B' */
                  ? Head extends Numbers
                    ? never /* stop union type forming */
                    : Head extends `${string}${Numbers}`
                      ? never /* stop union type forming */
                      : `${Head}-${ToDash<Caps>}_${ToDash<Tail>}` /* 'aBCd' => `${'a'}_${Lowercase<'B'>}_${ToSnake<'Cd'>}` */
                  : `${ToDash<Head>}-${Lowercase<Caps>}${ToDash<Tail>}` /* 'aBcD' where Caps = 'B' tail starts as lowercase */
              : never
            : never
          : never
    : S /* 'abc'  */
    ...
    : T extends Array<infer ArrayType>
      ...
      : T extends Uint8Array
        ...
        : T extends Date
          ? Date
          : {
              [K in keyof T as ToSnake<K>]: T[K] extends Array<infer ArrayType> | undefined | null
                ? ArrayType extends object
                  ? Array<ObjectToSnake<ArrayType>>
                  : Array<ArrayType>
                : T[K] extends object | undefined | null
...
export type ObjectToDash<T extends object | undefined | null> = T extends undefined
  ? undefined
  : T extends null
    ? null
    : T extends Array<infer ArrayType>
      ? ArrayType extends object
        ? Array<ObjectToDash<ArrayType>>
        : Array<ArrayType>
      : T extends Uint8Array
        ? Uint8Array
        : T extends Date
          ? Date
          : {
              [K in keyof T as ToDash<K>]: T[K] extends Array<infer ArrayType> | undefined | null
                ? ArrayType extends object
                  ? Array<ObjectToDash<ArrayType>>
                  : Array<ArrayType>
                : T[K] extends object | undefined | null
